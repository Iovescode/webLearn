<template>
  <div class="hello">
    ...
    <!-- form card -->
    <div v-if="isShow">
      <div class="box-card" v-for="(item,index) in makeData" :key="item.index" v-if="item.formShow">
      <div slot="header" class="clearfix"><span>{{item.formName}}</span></div>
         <el-form  :inline="true" :ref="item.formRef" :model="item.formModel" class="demo-form-inline">
            <el-form-item label="审批人" prop='user' :rules="[{ required: true, message: '审批人不能为空'}]">
              <vue-timepicker :minute-interval="5" :format="yourFormat" v-model="item.formModel.user" @change="kl"></vue-timepicker>
              <p v-if="item.formModel.aa">dcdvcfd</p>
      ...
      makeData: [
        {
          ...
          formModel: { user: {"hh": "","mm": ""}, region: "" }, //表单model
          formShow: "", //表单是否显示的控制
          formName: "表单A" //表单标题
        },
        {
          formRef: "formSecond",
          formModel: { user: {"hh": "","mm": ""}, region: "" },
          formShow: "",
          formName: "表单B"
        },
        {
  ...
  methods: {
    kl(){
      this.makeData.map((item, index) => {
        if(item.formModel.user.hh){
          item.formModel.aa=false
        }else{
           item.formModel.aa=true
        }
      })
    },
    enterForm() {
      ...
      this.makeData.forEach((item, index) => {
        // console.log(item.formModel.user.hh)
        //将有权限的表单做校验等等
        if (item.formShow) {
      ...
      function checkForm(arrName) {
        ...
        var result = new Promise(function(resolve, reject) {
          _self.$refs[arrName][0].validate(valid => {
            console.log(_self.$refs[arrName][0])
            if (valid) {
              resolve();
            } else {
              reject();
